...
class BossddPlatform(
    ...
    override fun doDataExtracting(subscription: Subscription) {
        ...
        val resultPredicate: (String) -> Boolean = {
            it.toJsonWrapper().getInt("code") == 0
        }
        val jobList = JSONArray()
        repeat(3) { i ->
            val res = if(i < 1) {
                browserService.waitForResponse(url, apiUrl, resultPredicate)
            } else {
                val jsExpression = "document.querySelector('.job-list-container').scrollBy(0, 5000)"
                browserService.waitForResponseByJs(jsExpression, apiUrl, resultPredicate)
            }
            jobList.addAll(res.toJsonWrapper().getArray("zpData.jobList"))
            TimeUnit.SECONDS.sleep(3)
        }
        jobList.forEachWrapper {
