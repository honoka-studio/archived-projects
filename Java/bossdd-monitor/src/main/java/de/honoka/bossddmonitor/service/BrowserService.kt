...
class BrowserService(
    ...
    private fun closeBrowser() {
        ...
        browserOrNull?.runCatching {
            tryBlock(3) {
                devTools.close()
                quit()
                RuntimeUtilsExt.exec {
                    win("taskkill", "/f", "/im", "chromedriver.exe")
                }
            }
        }?.getOrElse {
        ...
        browserProcess?.runCatching {
            tryBlock(3) {
                ...
                if(isAlive && browserProperties.stopProcessByCommand) {
                    RuntimeUtilsExt.exec {
    ...
    private fun loadPage(url: String, waitMillisAfterLoad: Long = 0) {
        ...
        tryBlock(3) {
            runCatching {
                ...
                val shouldReinit = cause.message?.contains("ERR_TUNNEL_CONNECTION_FAILED") != true
    ...
    fun waitForResponse(
        urlToLoad: String,
        urlPrefixToWait: String,
        resultPredicate: ((String) -> Boolean)? = null
    ) = doWaitForResponse(urlToLoad, null, urlPrefixToWait, resultPredicate)

    @Synchronized
    fun waitForResponseByJs(
        @Language("JavaScript") jsExpression: String,
        urlPrefixToWait: String,
        resultPredicate: ((String) -> Boolean)? = null
    ) = doWaitForResponse(null, jsExpression, urlPrefixToWait, resultPredicate)

    private fun doWaitForResponse(
        urlToLoad: String?,
        jsExpression: String?,
        urlPrefixToWait: String,
        resultPredicate: ((String) -> Boolean)? = null
        ...
        val action = Callable {
            ...
            when {
                urlToLoad != null -> loadPage(urlToLoad)
                jsExpression != null -> browser.executeScript(jsExpression)
            }
            outer@
            for(i in 1..120) {
                Thread.sleep(500)
